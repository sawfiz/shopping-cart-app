...
const App = () => {
  ...
  const inventory = useRef(products.map(product => ({...product, uid: uniqid()})))
  ...
  return (
    <>
      ...
      <Routes>
        ...
        <Route
          path="/product/:id"
          element={<ProductDetails inventory={inventory.current} addToCart={addToCart} />}
        />
        <Route path="/cart" element={<Cart cart={cart} updateOrder={updateOrder}/>} />
      </Routes>
    </>
  );
};
